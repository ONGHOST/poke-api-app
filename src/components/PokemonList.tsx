...
import { fetchPokemonListRequest } from '../redux/actions';
import PokemonModal from './PokemonModal';
import InfiniteScroll from 'react-infinite-scroll-component';

const PokemonList: React.FC = () => {
  const dispatch = useDispatch();
  const { data, loading, error } = useSelector((state: RootState) => state.pokemonList);
  ...
  useEffect(() => {
    dispatch(fetchPokemonListRequest(offset, 10)); // Fetch initial data
  }, []);
  // dispatch, offset
  const fetchMoreData = () => {
    setOffset(offset + 3); // Fetch more data when scrolled to bottom
  };
        ...
        <div className='flex-1 max-w-4x1 mx-auto p-10'>
          <ul className='grid grid-cols-4 gap-4'>
            {data.map((pokemon, index) => (
              <li
                className='bg-bleu rounded-lg shadow-xl'
                key={index}
                ...
                <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                  <div className="sm:flex sm:items-start">
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      <h3 className="text-base font-semibold leading-6 text-gray-900" id="modal-title">{pokemon.name}</h3>
                      <div className="mt-2">
          ...
          {modalOpen && <PokemonModal selectedPokemon={selectedPokemon} closeModal={closeModal} />}
        </div>
      </div>
    </InfiniteScroll>
  );
